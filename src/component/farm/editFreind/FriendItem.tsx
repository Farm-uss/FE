...
const FriendItem = ({ member, onRemove, myRole }: FriendItemProps) => {
    ...
    <div className="flex items-center gap-4 w-full">
      ...
      <div className="flex-1 bg-[#E8E2D5] rounded-2xl h-20 flex items-center justify-between pl-3 pr-4 shadow-sm min-w-0">
        ...
        <div className="flex-none min-w-[80px] flex justify-end">
          {member.role === 'OWNER' ? (
            /* 이 항목이 방장일 때 (방장 뱃지 표시) */
            <div className="bg-[#648E2E] w-[68px] text-white text-c-12b rounded-full h-12 flex items-center justify-center shadow-sm font-bold">
              방장
            </div>
          ) : (
            /*  이 항목이 일반 멤버일 때 */
            /* 내가 방장(OWNER)인 경우에만 '내보내기' 버튼 노출 */
            myRole === 'OWNER' && (
              <button
                onClick={() => onRemove(member.userId)}
                className="bg-[#20110A] w-[68px] text-white text-c-12b rounded-full h-12 active:scale-95 transition-transform font-bold"
              >
                내보내기
              </button>
            )
