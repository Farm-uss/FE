...
const FarmManageCard = ({
  ...
  return (
    <div
      className="w-full rounded-[22px] border-[1.5px] border-dashed border-[#B9B0A1] bg-[#F3EFE6] p-[16px] cursor-pointer active:scale-[0.99] transition-transform"
          ...
          {!isEditing ? (
            ...
            <div className="flex flex-col h-full justify-center gap-[14px]">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onShowModal(`${farm.name} 수정 화면으로 이동합니다.`);
                }}
              >
...
const ManageFarmsPage = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setModal({ title: '농장이 삭제되었습니다.' });
    } catch (error) {
      console.error('농장 삭제 실패:', error);
      setModal({
        title: '농장 삭제에 실패했습니다.',
        description: '삭제 권한이 있는지 확인해주세요.',
      });
    } finally {
      setIsDeleting(false);
          ...
          <div className="flex flex-col gap-[16px]">
            {farmList.map((farm) => (
              <FarmManageCard
                ...
                onShowModal={(title) => setModal({ title })}
              />
            ))}
            <AddFarmCard />
          </div>
